...
1. In AWS Console, go to EC2 > Launch Instance. 
    ...
    Application and OS Images (AMI): Select MyAMIs > Owned by me > ee3801_part2_lab8_ami
...
9. In your browser, go to <a href="http://<ip_address>:8080">http://<ip_address>:8080</a>. Login user: airflow, password: *******.
...
3. Create Table by right click on Database > carpark_system > Schemas > public > Tables. Save.
...
1. Download the <a href="./generateCars_insertPostgresql.py">generateCars_insertPostgresql.py</a> and <a href="./readPostgressql_writeElasticsearch.py">readPostgressql_writeElasticsearch.py</a>. Edit the passwords and copy the files inside ~/dev_airflow/dags/ using the command:
    ...
    ```
    $ scp -i MyKeyPair.pem *.py ec2-user@<ip_address>:~/Documents/projects/ee3801/dev_airflow/dags/
    ```
